...
tracker = tracking.MLFlowTracker("Baseline")
...
for batch in data_loader_test:
    ...
    for seq in range(1, gseq_masked.shape[0]):
        non_zero_indices = np.where((gseq_masked[seq-1] != 0).all(axis=1))[0]
        f = interpolate.interp1d(non_zero_indices, gseq_masked[seq-1][non_zero_indices], axis=0, kind='linear', fill_value="extrapolate")
        grecon = f(np.arange(len(gseq_masked[seq])))
        #mask = np.isnan(grecon)
        #idx = np.where(~mask,np.arange(mask.shape[1]),0)
        #grecon[mask] = grecon[np.nonzero(mask)[0], idx[mask]]
        gestimate = np.concatenate((gestimate, np.expand_dims(grecon, axis=0)), axis=0)

estimate_test_sph, actual_test_sph = conv_spherical(gestimate), conv_spherical(gseqs[1:])
    
tracker.log_metrics("Test Euclidean Distance", compute_euclidean_distance(estimate_test_sph, actual_test_sph))
#tracker.log_metrics("Test Dynamic Time Warping Gaze", compute_dtw_metric(estimate_test_sph, actual_test_sph))
