...
class LIBERTDataset4Pretrain(Dataset):
    ...
    def __getitem__(self, index):
        instance = self.data[index] # 1 sequence
        for proc in self.pipeline:
            instance = proc(instance)
...
def handle_argv(target, config_train, prefix):
    ...
    parser.add_argument('dataset', type=str, help='Dataset name', choices=['hhar', 'motion', 'uci', 'shoaib', 'hgbd'])
    parser.add_argument('dataset_version',  type=str, help='Dataset version', choices=['10_100', '20_120', '2'])
